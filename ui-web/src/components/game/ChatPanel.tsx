 ...
 * el nombre; si no (partidas/mensajes antiguos sin este prefijo), se cae de
 * vuelta a la etiqueta genérica "Jugador". Las tiradas del botón "Tirar
 * Dados" también llevan ya este prefijo ("**Sandra:** 🎲 tira 1d20: **5**").
 */
    ...
    synthesizeSpeech.mutate(
      { text: stripMarkdownForSpeech(content) },
      {
        onSuccess: ({ audioBase64 }) => {
          ...
          // Sin esto, si el navegador bloquea la reproducción (Safari/iPad
          // tras una petición asíncrona) o el audio no se puede decodificar,
          // el botón se quedaba en ⏹ para siempre.
          audio.addEventListener('error', () => setSpeakingIndex(null));
