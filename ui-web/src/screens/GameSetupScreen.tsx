...
export function GameSetupScreen() {
  const { data: games, isLoading, error: gamesError } = useMyGames();
  ...
  const handleJoinByCode = (e: FormEvent) => {
    ...
    if (!trimmed) return;
    // Los códigos se generan en mayúsculas (RandomGameCodeGenerator) y la
    // búsqueda es exacta: "k7m2qx" daba "Partida no encontrada". Los ids UUID
    // de partidas antiguas (con guiones) se dejan tal cual.
    const code = /^[a-z0-9]{4,8}$/i.test(trimmed) ? trimmed.toUpperCase() : trimmed;
      ...
      <div className="full-bleed-card game-setup-card">
        ...
        <div className="game-setup-columns">
          <div className="game-setup-col">
            ...
            {gamesError && <p className="error-msg">No se pudieron cargar tus partidas: {gamesError.message}</p>}
