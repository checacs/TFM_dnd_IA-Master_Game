...
import { INTERNAL_API_KEY_HEADER } from './internal-api-key';
...
export class McpToolCaller implements ToolCaller {
  ...
  /** internalApiKey: secreto compartido con la API (INTERNAL_API_KEY) -- /mcp lo exige si está configurado. */
  constructor(
  ...
  private async getClient(): Promise<Client> {
    ...
    if (!this.connecting) {
      ...
      const transport = new StreamableHTTPClientTransport(new URL(this.serverUrl), {
        requestInit: this.internalApiKey ? { headers: { [INTERNAL_API_KEY_HEADER]: this.internalApiKey } } : undefined,
      });
      this.connecting = withTimeout(client.connect(transport), 'connect')
          .then(() => {
            this.client = client;
            return client;
          })
  ...
  private async withClient<T>(label: string, fn: (client: Client) => Promise<T>): Promise<T> {
    ...
    try {
      ...
      // OJO: no se llama a client.close() aquí -- el cliente es compartido por
      // todas las partidas, y cerrarlo abortaría las llamadas en vuelo de
      // OTROS turnos (que pueden haber mutado ya la partida en la API).
      this.client = null;
      this.connecting = null;
      throw error;
    }
