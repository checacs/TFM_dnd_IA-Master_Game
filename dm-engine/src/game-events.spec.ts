...
describe('toGameEvent', () => {
  ...
  it('mapea resolve_enemy_attack a ataque_resuelto (la UI lo trata igual que cualquier ataque)', () => {
    const event = toGameEvent('resolve_enemy_attack', { hit: false, damage: 0 });
    expect(event).toEqual({ type: 'ataque_resuelto', payload: { hit: false, damage: 0 } });
  });

  it('mapea resolve_player_attack a ataque_resuelto', () => {
    const event = toGameEvent('resolve_player_attack', { hit: true, damage: 9 });
    expect(event).toEqual({ type: 'ataque_resuelto', payload: { hit: true, damage: 9 } });
  });
