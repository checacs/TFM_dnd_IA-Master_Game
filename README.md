...
| **`ui-web`** | React 18 + TypeScript + Vite, TanStack Query (estado servidor / polling), React Router |
...
### Opción rápida: todo con Docker Compose

Levanta MongoDB, la API + MCP, dm-engine y ui-web con un solo comando (necesitas Docker Desktop). Reutiliza los `.env` de cada proyecto, así que antes rellena `API_REST TFM/.env` (al menos `JWT_SECRET`) y `dm-engine/.env` (`DEEPSEEK_API_KEY`, `DEEPSEEK_MODEL`).

```bash
docker compose up --build -d                  # arranca todo
docker compose --profile seed run --rm seed   # solo la 1ª vez: usuarios, mapas y catálogo SRD (varios minutos)
```

| Servicio | URL |
|---|---|
| UI web | http://localhost:3001 |
| API REST + MCP | http://localhost:3000 |
| MongoDB (Compass) | mongodb://localhost:27018 |

- La base de datos es la del contenedor `mongo` (volumen `mongo-data`), **no** la de producción.
- `docker compose down` para parar (los datos se conservan); `docker compose down -v` los borra.
- `docker compose logs -f api dm-engine` para seguir los turnos del DM.
- La app móvil se arranca con Expo en el PC y se conecta sola a la API del compose: ver la sección siguiente.
- Variables propias del compose (`INTERNAL_API_KEY`, `MONGODB_URI`): ver `.env.example` en la raíz.

### Jugar desde el móvil en local

Con el compose arrancado, el móvil (conectado a la misma Wi-Fi que el PC) puede jugar contra tu API local. `mobile-app` detecta sola la IP del PC: no hace falta configurar nada mientras no definas `EXPO_PUBLIC_API_URL`.

1. En el PC, dentro de `mobile-app`: `npm install` (solo la primera vez) y después `npx expo start`. En la consola aparece la IP del PC (`exp://192.168.X.X:8081`); también la ves con `ipconfig` → "Dirección IPv4".
2. En el móvil, una de estas dos opciones:
   - **Sin instalar nada:** pulsa `w` en la consola de Expo (modo web) y abre en el navegador del móvil `http://IP-DEL-PC:8081`.
   - **Con la app Expo Go** (Play Store / App Store): escanea el QR que muestra la consola. Si Expo Go dice que el proyecto no es compatible con su versión de SDK, usa la opción del navegador.
3. La primera vez, Windows pregunta si deja a Node.js y a Docker aceptar conexiones: permítelo en **redes privadas** (y comprueba que tu Wi-Fi está marcada como red privada).

Para comprobar que el móvil llega a la API, abre `http://IP-DEL-PC:3000/games` en su navegador: si responde con un error 401 en JSON, la conexión funciona.

### `API_REST TFM` (backend + servidor MCP)

```bash
cd API_REST TFM
cp .env.example .env    # rellena MONGODB_URI, JWT_SECRET, DM_ENGINE_URL, INTERNAL_API_KEY
npm install
...
```bash
cd dm-engine
cp .env.example .env    # rellena DEEPSEEK_API_KEY, DEEPSEEK_MODEL, MCP_SERVER_URL, INTERNAL_API_KEY (el mismo valor que en la API)
npm install
npm test
...
cd ui-web
npm install
npm run dev                # http://localhost:3001 — espera la API en http://localhost:3000
```
...
La app detecta sola la IP del PC que sirve el bundle (Expo Go) o desde el que se abre la página (modo web) y llama a la API en el puerto 3000 de esa IP — ver "Jugar desde el móvil en local". Para apuntar a otra API, define `EXPO_PUBLIC_API_URL` (así lo hacen los perfiles de `eas.json` para Render):

```bash
EXPO_PUBLIC_API_URL=https://mi-api.example.com npx expo start
```
...
Detectados durante el diseño, con su estado actual:

- **Servidor MCP (04):** verificar el id exacto del modelo de DeepSeek vigente en `api-docs.deepseek.com` antes de desplegar — cambia con frecuencia (`DEEPSEEK_MODEL`, sin valor por defecto a propósito).
- **Motor IA - DM (05):** ✅ el bucle de tool-calling tiene límite de iteraciones (`MAX_TOOL_CALL_ITERATIONS`) y de rondas de corrección.
- **Arquitectura Clean (03):** ⚠️ `main.ts` ya publica el contrato OpenAPI en `/api-docs` (y `/api-docs-json`) en cuanto `@nestjs/swagger` está instalado — falta `npm i @nestjs/swagger` en `API_REST TFM` y desplegar.
- **Autenticación (10):** ✅ `LaunchGameUseCase` (no `StartGameUseCase`, para no confundirlo con la tool MCP `start_combat`).
- **Autenticación (10):** ✅ `LevelUpUseCase` comprueba que el personaje pertenece al usuario autenticado.
- **Seguridad API ↔ dm-engine:** `/mcp` (API) y `/turn` (dm-engine) exigen la cabecera `x-internal-api-key` con el valor de `INTERNAL_API_KEY`, que debe ser **el mismo** en ambos servicios de Render. Sin la variable, los dos endpoints quedan abiertos (solo aceptable en local).
