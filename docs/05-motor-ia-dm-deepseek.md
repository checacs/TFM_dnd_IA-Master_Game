...
| `resolve_player_attack` | `ataque_resuelto` | Ataque con arma de un jugador con su tirada de "Tirar Dados" (arma, modificador y CA reales; la tirada queda en el chat) |
| `resolve_enemy_attack` | `ataque_resuelto` | Igual, para el ataque de un enemigo en su fase (datos del catálogo y armadura real del personaje; la tirada del DM queda en el chat) |
...
- **Acción de un jugador en combate** → el jugador reclama el candado con "Mi turno" (`POST /games/:id/claim-turn`) y, cuando quiere actuar, escribe su acción (`POST /games/:id/player-action`). Ese endpoint es el que dispara `runDmTurn` — el DM-IA narra la acción de ESE jugador, y solo cuando `roundPhase` pasa a "enemigos" (todos los jugadores vivos ya actuaron) el DM-IA resuelve él mismo, en ese mismo turno, los ataques de los enemigos con `resolve_enemy_attack` y termina llamando a `advance_to_player_round`. Si el turno acaba con la fase de enemigos abierta, dm-engine tira por cada enemigo vivo que no haya atacado (`resolve_enemy_attack`) antes de reabrir la ronda, y cuando el turno pasa de un jugador a otro añade al final "👉 **Nombre**, es tu turno" si el DM no se ha dirigido ya a quien le toca. Si el DM narra un ataque de enemigo sin tirada (o fuera de su fase), o escribe él mismo una línea de tirada, se le obliga a corregirlo. Si un jugador tira ("**Nombre:** 🎲 tira 1d20: **N**") y el DM no resuelve la tirada, se le pide `resolve_player_attack` con esa tirada; si aun así no lo hace, dm-engine la resuelve contra el enemigo mencionado más recientemente, cierra el turno del jugador y sustituye la narración inventada por una coherente con el resultado real. Si el DM da el turno a un jugador que ya actuó en la ronda, se le corrige (y, si insiste, el chat aclara a quién le toca).
