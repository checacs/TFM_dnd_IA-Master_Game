...
> **Nota de implementación (estado real):** la estructura de arriba es la del diseño inicial. En la versión implementada ui-web es una pantalla de **solo lectura** (las acciones de juego se hacen desde `mobile-app`): los hooks viven todos en `src/api/hooks.ts`, no hay `DiceRollPanel` ni `SaveGameButton`, y la única acción de partida que lanza ui-web es el arranque de la escena inicial (`useStartOpeningScene` → `POST /games/:id/message`, cuyo mensaje construye el servidor). Las rutas `/attack` y `/start-combat` son del DM-IA (vía MCP) y por REST quedan solo para administradores.
