...
interface BattleMap {
  ...
  zones: MapZone[];            // salas con nombre y rango de celdas (place_participant)
  connections: { mapId: string; via: string }[]; // salidas reales a otros mapas (escalera, trampilla...)
  closedExits: boolean;        // true: connections son sus ÚNICAS salidas
}
```
...
`connections`/`closedExits` se añadieron tras un caso real: desde el sótano de la taberna el DM-IA inventó una "puerta de hierro" y aplicó una fortaleza entera. Si el mapa actual de la partida (último de `mapHistory`) tiene `closedExits: true`, `set_battle_map` y `start_combat` rechazan cualquier `mapId` que no esté en sus `connections` (`BattleMap.assertCanLeadTo`). `describe_map` las devuelve para que el DM sepa a dónde lleva cada salida.
