...
- Enemigo a 0 HP → derrotado. Jugador a 0 HP → inconsciente: no puede reclamar turno ni actuar. `resolve_attack` y `cast_spell` devuelven `targetRemainingHp`/`targetDefeated` y el chat anota "X cae derrotado", para que el DM-IA no narre en pie a un objetivo a 0 HP.
- **Encuentros equilibrados:** `start_combat` aplica el presupuesto de XP de la Guía del DM 2024: suma la XP por CR de los enemigos (sin multiplicador por número de monstruos) y rechaza el encuentro si supera el presupuesto de dificultad "alta" del grupo según el nivel real de cada personaje (100 XP por personaje de nivel 1, 200 de nivel 2, 400 de nivel 3...; `domain/services/encounter-difficulty.ts`). Caso real: 2 Ghouls + 1 Zombie contra dos personajes de nivel 1 (450 XP contra un límite de 200). Una primera versión con la guía de 2014 (con multiplicador) resultó demasiado estricta con grupos de enemigos débiles: tres goblins salían "mortales" y el DM narraba tres goblins de los que solo existía uno.
- **Cambio respecto al diseño original:** si *todo* el grupo llega a 0 HP, la partida pasa a `finalizada` de forma automática y permanente (`Game.checkForPartyWipe`). En partidas reales el DM-IA no sabía cerrar la historia ante un grupo entero caído y acababa en el mensaje de error genérico; el cierre determinista lo evita. El MVP no tiene curación ni resurrección.
