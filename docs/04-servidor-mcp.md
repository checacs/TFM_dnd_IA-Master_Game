...
export function registerGameTools(server: McpServer, tools: GameMcpTools): void {
  ...
  // NUEVA — ataque de un enemigo en su fase con datos reales (sin números del LLM).
  server.tool('resolve_enemy_attack', '...', {
    gameId: z.string(), enemyInstanceId: z.string(), targetCharacterId: z.string(),
  }, async (input) => ({
    content: [{ type: 'text', text: JSON.stringify(await tools.resolveEnemyAttackTool(input)) }],
  }));
  ...
  server.tool('start_combat',
    ...
    'Rechaza encuentros por encima del umbral "mortal" del grupo y mapas no conectados con el actual.',
...
- El endpoint `/mcp` exige la cabecera `x-internal-api-key` con el secreto compartido `INTERNAL_API_KEY` (mismo valor en la API y en `dm-engine`). En Render (plan gratuito) no hay red privada, así que la API es pública y `/mcp` quedaba abierto a cualquiera; el secreto compartido sustituye a la "red de confianza" del diseño original. El único cliente esperado sigue siendo `dm-engine` — sigue siendo así incluso ahora que `dm-engine` usa DeepSeek en vez de la API de Anthropic (si se hubiera usado el conector MCP nativo de Anthropic, `/mcp` habría tenido que exponerse públicamente; con DeepSeek, `dm-engine` es su propio cliente MCP y la conexión se queda interna).
