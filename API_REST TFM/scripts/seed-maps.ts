...
const maps = [
  ...
  },
  {
    ...
    // Sin closedExits: la puerta principal da a la calle, desde aquí se puede ir a cualquier sitio.
  ...
  },
  {
    ...
    description: 'Sistema de cuevas conectado por un río, con estanque oculto, gruta de cristales y guarida de hongos. ' +
        'Un túnel estrecho sube hasta la trampilla del sótano de la taberna (mapId sotanoTaberna).',
    tags: ['exterior', 'cueva', 'subterraneo'],
    // Sin closedExits: el Puesto de Guardia da al exterior.
    connections: [{ mapId: 'sotanoTaberna', via: 'el túnel que sube hasta la trampilla del sótano de la taberna' }],
  ...
  },
  {
    ...
    description: 'Variante de la cueva del río: las Cascadas del Norte, el Abismo Torrencial, el Estanque ' +
        ...
        'por la corriente que cruza la Caverna Principal hasta el Puesto de Guardia de la entrada. Un túnel ' +
        'estrecho sube hasta la trampilla del sótano de la taberna (mapId sotanoTaberna).',
    tags: ['exterior', 'cueva', 'subterraneo'],
    connections: [{ mapId: 'sotanoTaberna', via: 'el túnel que sube hasta la trampilla del sótano de la taberna' }],
  ...
  },
  {
    ...
    description: 'Sótano de almacenaje de la taberna, conectado a la sala principal (mapId tabernaMercenarios) ' +
        ...
        'de madera desmontable junto a una trampilla candada que baja por un túnel húmedo a las cuevas del río ' +
        'subterráneo (mapId cueva-rio o cueva-rio-alt). No tiene ninguna otra salida: ni puertas a la calle ni ' +
        'otros pasadizos.',
    tags: ['interior', 'taberna', 'sotano', 'almacen', 'multisala', 'escaleras'],
    // CASO REAL: el DM inventó una "puerta de hierro" en el sótano y aplicó
    // cripta-multisala (una fortaleza entera). Con closedExits, set_battle_map
    // y start_combat solo aceptan estas salidas reales.
    connections: [
    ...
    // inferior" son props dentro de Almacén del Sótano 2 (sin zona propia):
    // la trampilla es la conexión con las cuevas del río (ver connections).
    rows: 30,
    cols: 20,
...
async function seed() {
  const uri = process.env.MONGODB_URI || process.env.URL; // || (no ??): un "MONGODB_URI=" vacío en .env cae a URL
  if (!uri) {
