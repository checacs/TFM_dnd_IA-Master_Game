...
**Estado:** v2.0 — actualizado para reflejar la implementación real (el diseño original era del paso 4; desde entonces se añadieron mapas de combate y el catálogo de hechizos, y se corrigió un fallo real de arranque detectado al probarlo en vivo)
**Se apoya en:** `03-arquitectura-clean-api-nestjs.md`
**Tools reales:** 17 (fueron creciendo desde las 6 originales; la última, `advance_to_player_round`, se añadió al sustituir la iniciativa por rondas)
...
- Las 17 tools están registradas y responden a `tools/list` (verificado con el inspector oficial de MCP: `npx @modelcontextprotocol/inspector`, transporte "Streamable HTTP", conectado a `http://localhost:3000/mcp`).
...
*La hoja de ruta original (pasos 1-10) está completa. Los desarrollos posteriores (mapas, hechizos, el fallo de instancia compartida) se añadieron sobre esa base y quedan reflejados aquí.*