...
describe('ResolveAttackUseCase', () => {
  ...
  // Caso real: el DM narraba "malherida, tambaleándose" a una rata que el
  // sistema ya había dejado a 0 HP -- las tools de ataque no le decían al
  // modelo cuánta vida le quedaba al objetivo ni si había caído.
  describe('estado del objetivo tras el golpe', () => {
    it('devuelve el HP que le queda al objetivo y si ha caído', async () => {
      const { game, repo } = buildGameWithActiveEnemy();
      const useCase = new ResolveAttackUseCase(new FakeDiceRoller([15, 3]), repo);

      const result = await useCase.execute({
        gameId: game.id, targetId: 'enc-1-goblin-a', attackerName: 'Elyndra',
        attackerModifier: 2, targetArmorClass: 12, damageDice: '1d6',
      });

      expect(result.targetRemainingHp).toBe(4);
      expect(result.targetDefeated).toBe(false);
    });

    it('si el golpe lo deja a 0 HP, lo marca como derrotado y lo anuncia en el chat', async () => {
      const { game, repo } = buildGameWithActiveEnemy();
      const useCase = new ResolveAttackUseCase(new FakeDiceRoller([15, 9]), repo);

      const result = await useCase.execute({
        gameId: game.id, targetId: 'enc-1-goblin-a', attackerName: 'Elyndra',
        attackerModifier: 2, targetArmorClass: 12, damageDice: '1d6+3',
      });

      expect(result.targetRemainingHp).toBe(0);
      expect(result.targetDefeated).toBe(true);
      const log = (await repo.findById(game.id))!.toSnapshot().narrativeLog;
