...
import { SendMessageUseCase, MAX_DM_HISTORY_MESSAGES } from './send-message.use-case';
...
describe('SendMessageUseCase', () => {
  ...
  // Depuración: dos acciones casi simultáneas (varios jugadores con turno
  // reclamado a la vez) disparaban dos turnos del DM sobre la misma partida.
  it('rechaza un segundo turno de la misma partida mientras el primero sigue en curso, sin anotar su mensaje', async () => {
    const games = new FakeGameRepository();
    const game = Game.create({ name: 'La torre olvidada', hostUserId: 'host-1', maxPlayers: 4 });
    games.seed(game);
    ...
    const firstTurnStarted = new Promise<void>((r) => { signalFirstTurnStarted = r; });
    const firstDmEngine = new FakeDmEngineClient({ narrative: 'Primera respuesta.', events: [] }, async () => {
      signalFirstTurnStarted();
      await new Promise<void>((r) => { releaseFirstTurn = r; });
    });
    const firstTurn = new SendMessageUseCase(games, firstDmEngine).execute({
      gameId: game.id,
      messages: [{ role: 'user', content: 'Ataco al orco' }],
    });
    await firstTurnStarted;

    const secondDmEngine = new FakeDmEngineClient({ narrative: 'Segunda respuesta.', events: [] });
    const secondUseCase = new SendMessageUseCase(games, secondDmEngine);
    await expect(
      secondUseCase.execute({ gameId: game.id, messages: [{ role: 'user', content: 'Lanzo un hechizo' }] }),
    ).rejects.toThrow('El Máster todavía está respondiendo');
    expect(secondDmEngine.receivedMessages).toBeNull();
    ...
    expect(log).toEqual([
      { role: 'user', content: 'Ataco al orco' },
      { role: 'assistant', content: 'Primera respuesta.' },
    ]);
  });

  // Depuración: se enviaba el narrativeLog ENTERO en cada turno. dm-engine
  // acepta como mucho ~100 KB por petición (express.json por defecto), así que
  // tras unas decenas de turnos /turn respondía 413 para siempre y la partida
  // quedaba inservible.
  it('solo envía a dm-engine los últimos MAX_DM_HISTORY_MESSAGES mensajes del historial', async () => {
    const games = new FakeGameRepository();
    const game = Game.create({ name: 'La torre olvidada', hostUserId: 'host-1', maxPlayers: 4 });
    games.seed(game);
    ...
    const messages: DmEngineChatMessage[] = Array.from({ length: MAX_DM_HISTORY_MESSAGES + 25 }, (_, i) => ({
      role: i % 2 === 0 ? 'user' : 'assistant',
      content: `mensaje ${i}`,
    }));
    messages.push({ role: 'user', content: 'última acción' });

    await useCase.execute({ gameId: game.id, messages });

    expect(dmEngine.receivedMessages).toHaveLength(MAX_DM_HISTORY_MESSAGES);
    expect(dmEngine.receivedMessages![MAX_DM_HISTORY_MESSAGES - 1]).toEqual({ role: 'user', content: 'última acción' });
  });

  it('si falla el guardado final, intenta al menos cerrar el turno (dmTurnInProgress) antes de propagar el error', async () => {
    class FailingFinalSaveRepository extends FakeGameRepository {
      private saves = 0;
      async save(game: Game): Promise<void> {
        this.saves += 1;
        if (this.saves === 2) {
    ...
    const games = new FailingFinalSaveRepository();
    const game = Game.create({ name: 'La torre olvidada', hostUserId: 'host-1', maxPlayers: 4 });
    games.seed(game);
    const useCase = new SendMessageUseCase(games, new FakeDmEngineClient({ narrative: 'Ok.', events: [] }));

    await expect(useCase.execute({ gameId: game.id, messages: [{ role: 'user', content: 'Hola' }] })).rejects.toThrow('Mongo no disponible');

    expect((await games.findById(game.id))!.toSnapshot().dmTurnInProgress).toBe(false);
  });

  it('la ventana de historial nunca empieza por un mensaje del DM (algunos modelos lo rechazan)', async () => {
    const games = new FakeGameRepository();
    const game = Game.create({ name: 'La torre olvidada', hostUserId: 'host-1', maxPlayers: 4 });
    games.seed(game);
    const dmEngine = new FakeDmEngineClient({ narrative: 'Ok.', events: [] });
    ...
    // 61 mensajes alternos empezando por user: el recorte a 60 dejaría uno del DM delante.
    const messages: DmEngineChatMessage[] = Array.from({ length: MAX_DM_HISTORY_MESSAGES + 1 }, (_, i) => ({
      role: i % 2 === 0 ? 'user' : 'assistant',
      content: `mensaje ${i}`,
    }));
    ...
    expect(dmEngine.receivedMessages![0].role).toBe('user');
    expect(dmEngine.receivedMessages![dmEngine.receivedMessages!.length - 1]).toEqual(messages[messages.length - 1]);
  });
});
