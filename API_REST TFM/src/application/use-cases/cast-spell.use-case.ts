...
import { participantHp } from './resolve-attack.use-case';
...
export interface CastSpellResult {
  ...
  /** HP que le quedan al objetivo tras el hechizo (null si el hechizo no hace daño). */
  targetRemainingHp: number | null;
  /** true si el hechizo ha dejado al objetivo a 0 HP (ver AttackResult.targetDefeated). */
...
export class CastSpellUseCase {
  ...
  async execute(input: CastSpellInput): Promise<CastSpellResult> {
    ...
    // El conjurador tiene que ser jugador de ESTA partida: antes se podía
    // usar un personaje de otra partida para dañar a estos enemigos.
    if (!game.toSnapshot().players.some((p) => p.characterId === input.casterCharacterId)) {
    ...
    if (!spellSnapshot.damageType) {
      ...
      return {
        spellName: spellSnapshot.name,
        damageDealt: 0,
        targetSavedThrow: null,
    ...
    game.appendNarrativeEntry({
      role: 'assistant',
      content: this.describeSpellDamage(
          game, casterName, spellSnapshot.name, input.targetId, dc, targetSavedThrow, damage,
          hpBefore > 0 && targetDefeated,
      ),
    });
    await this.games.save(game);

    return { spellName: spellSnapshot.name, damageDealt: damage, targetSavedThrow, targetRemainingHp, targetDefeated };
  }
  ...
  private describeSpellDamage(
    ...
    const defeatedText = justDefeated ? ` — **${targetName}** cae derrotado.` : '';

    if (dc === null) {
      // Sin tirada de salvación: impacto automático (simplificación del MVP).
      return `${header} — Daño: **${damage}**${defeatedText}`;
    }

    const saveText = targetSavedThrow ? `salva (CD ${dc})` : `falla la salvación (CD ${dc})`;
    return `${header}, ${saveText} — Daño: **${damage}**${defeatedText}`;
  }
}
