...
export interface AttackResult {
  ...
  damage: number;
  /**
   * HP que le quedan al objetivo DESPUÉS de este ataque. CASO REAL: el mago
   * dejó a una rata a 0 HP y el DM-IA siguió narrando que "se tambalea" --
   * solo recibía el daño y no sabía que el objetivo había caído. Con estos
   * dos campos el DM no tiene que adivinarlo.
   */
  targetRemainingHp: number;
  /** true si el objetivo está a 0 HP tras el ataque (derrotado/inconsciente). */
...
export class ResolveAttackUseCase {
  ...
  async execute(input: ResolveAttackInput): Promise<AttackResult> {
    ...
    const hpBefore = participantHp(game, input.targetId);
    if (hit) {
      game.applyDamageToParticipant(input.targetId, damage);
    }
    const targetRemainingHp = participantHp(game, input.targetId);
    const targetDefeated = targetRemainingHp <= 0;
    const justDefeated = hpBefore > 0 && targetDefeated;
    ...
    game.appendNarrativeEntry({
      role: 'assistant',
      content: this.describeRoll(game, input, attackRoll, hit, damage, justDefeated),
    });
    await this.games.save(game);

    return { hit, attackRoll, damage, targetRemainingHp, targetDefeated };
  }
  ...
  private describeRoll(
    ...
    const targetIsPlayer = snapshot.players.some((p) => p.characterId === input.targetId);
    const targetName =
    ...
    // Si el d20 vino de playerD20, se aclara que es la tirada del jugador.
    return formatAttackRollMessage({
      attackerName: input.attackerName,
      weaponName: input.weaponName,
      targetName,
      modifier: input.attackerModifier,
      ...
      damage,
      targetDown: justDefeated ? (targetIsPlayer ? 'inconsciente' : 'derrotado') : null,
    });
  }
}

/** HP actual de un jugador o enemigo de la partida (0 si no se encuentra). */
export function participantHp(game: Game, participantId: string): number {
  const snapshot = game.toSnapshot();
  const player = snapshot.players.find((p) => p.characterId === participantId);
  if (player) {
    return player.currentHp;
  }
  return snapshot.activeEncounter?.enemies.find((e) => e.instanceId === participantId)?.currentHp ?? 0;
}
