...
export class EquipItemUseCase {
  ...
  async execute(input: EquipItemInput): Promise<void> {
    ...
    if (item) {
      ...
      } else if (snapshot.category === 'Armor' && isShield(input.equipmentId, snapshot.name)) {
        // El escudo del SRD también es categoría "Armor" (armor_class.base = 2):
        // por equipArmor sustituía la CA en vez de sumar +2 (CA 12 -> 2).
        character.equipShield(input.equipmentId);
...
/**
 * El catálogo importado no guarda armor_category, así que el escudo se
 * reconoce por su id/nombre del SRD 2014 (el único escudo del catálogo de
 * equipo normal es "shield"; los escudos mágicos viven en otro catálogo).
 */
function isShield(equipmentId: string, name: string): boolean {
  return equipmentId === 'shield' || /^(shield|escudo)$/i.test(name.trim());
}
