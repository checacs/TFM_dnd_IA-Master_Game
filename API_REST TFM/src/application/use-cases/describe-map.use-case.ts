...
import { MapConnection, MapZone } from '../../domain/entities/battle-map.entity';
...
export interface DescribeMapResult {
   ...
   */
  zones: MapZone[];
  /** A qué mapId lleva cada salida real del mapa (escalera, trampilla...). */
  connections: MapConnection[];
  /** true: esas conexiones son las ÚNICAS salidas (set_battle_map rechaza cualquier otro mapa). */
  closedExits: boolean;
}
...
export class DescribeMapUseCase {
  ...
  async execute(input: DescribeMapInput): Promise<DescribeMapResult> {
    ...
    return {
      ...
      closedExits: snapshot.closedExits,
    };
  }
}
