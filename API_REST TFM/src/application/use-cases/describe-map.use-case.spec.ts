...
describe('DescribeMapUseCase', () => {
  ...
  it('incluye las conexiones del mapa (a qué mapId lleva cada paso) para que el DM no invente salidas', async () => {
    const sotano = BattleMap.create({
      name: 'Sótano de la Taberna', description: 'Sótano.', tags: ['sotano'], rows: 30, cols: 20,
      imageUrl: '/maps/battleMap17-sotanoTaberna.png',
      connections: [{ mapId: 'cueva-rio', via: 'la trampilla candada' }],
      closedExits: true,
    }, 'sotanoTaberna');
    const useCase = new DescribeMapUseCase(new FakeMapRepository([sotano]));

    const result = await useCase.execute({ mapId: 'sotanoTaberna' });

    expect(result.connections).toEqual([{ mapId: 'cueva-rio', via: 'la trampilla candada' }]);
    expect(result.closedExits).toBe(true);
  });
});
