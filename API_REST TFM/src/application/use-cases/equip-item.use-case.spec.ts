...
/** Escudo tal cual lo importa scripts/import-equipment.ts desde el SRD 2014. */
function buildShield() {
  return Equipment.create(
    {
      name: 'Shield', category: 'Armor', cost: { quantity: 10, unit: 'gp' }, weight: 6, description: '',
      weaponCategory: null, weaponRange: null, damageDice: null, damageType: null,
      properties: [], armorClass: { base: 2, dexBonus: false, maxBonus: null },
    },
    'shield',
  );
}
...
describe('EquipItemUseCase', () => {
  ...
  it('equipar un escudo suma +2 a la CA (antes la dejaba en 2) y no toca la armadura equipada', async () => {
    const characters = new FakeCharacterRepository();
    characters.seed(buildCharacterWithInventory([{ equipmentId: 'shield', name: 'Shield' }]));
    const useCase = new EquipItemUseCase(
      characters, new FakeEquipmentRepository([buildShield()]), new FakeMagicItemRepository([]),
    );

    await useCase.execute({ characterId: 'char-1', requestingUserId: 'user-1', equipmentId: 'shield' });

    const snapshot = (await characters.findById('char-1'))?.toSnapshot();
    expect(snapshot?.ac).toBe(14); // 12 + 2
