...
function buildMago(overrides: Partial<Parameters<typeof Character.create>[0]> = {}, id = 'char-1') {
...
describe('CastSpellUseCase', () => {
  ...
  // Depuración: no se comprobaba que el conjurador fuera jugador de ESA
  // partida -- con un personaje propio de la partida A se podía dañar a los
  // enemigos de la partida B (y gastar ranuras) llamando a su cast-spell.
  it('lanza DomainError si el conjurador no es jugador de esa partida', async () => {
    const { game, games } = buildGameWithGoblin();
    const characters = new FakeCharacterRepository();
    characters.seed(buildMago({ gameId: 'otra-partida' }, 'char-9'));
    const spells = new FakeSpellRepository([buildRayOfFrost()]);
    const enemies = new FakeEnemyRepository([buildGoblin()]);
    const useCase = new CastSpellUseCase(new FakeDiceRoller([6]), games, characters, spells, enemies);

    await expect(
      useCase.execute({ gameId: game.id, requestingUserId: 'user-1', casterCharacterId: 'char-9', spellId: 'ray-of-frost', targetId: 'enc-1-goblin-a' }),
    ).rejects.toThrow('no es un jugador de esta partida');
  });

  it('devuelve el HP que le queda al objetivo y si ha caído (Proyectil Mágico que remata a un goblin)', async () => {
    const { game, games } = buildGameWithGoblin();
    const characters = new FakeCharacterRepository();
    characters.seed(buildMago());
    const spells = new FakeSpellRepository([buildRayOfFrost()]);
    const enemies = new FakeEnemyRepository([buildGoblin()]);
    const useCase = new CastSpellUseCase(new FakeDiceRoller([99]), games, characters, spells, enemies);

    const result = await useCase.execute({
      gameId: game.id, casterCharacterId: 'char-1', spellId: 'ray-of-frost', targetId: 'enc-1-goblin-a',
    });

    expect(result.targetRemainingHp).toBe(0);
    expect(result.targetDefeated).toBe(true);
    const log = (await games.findById(game.id))!.toSnapshot().narrativeLog;
    expect(log[log.length - 1].content).toContain('cae derrotado');
  });
});
