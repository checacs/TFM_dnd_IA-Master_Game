...
describe('BattleMap', () => {
  ...
  describe('conexiones con otros mapas', () => {
    function buildSotano() {
      return BattleMap.create({
        name: 'Sótano de la Taberna', description: 'Sótano de almacenaje.', tags: ['sotano'],
        rows: 30, cols: 20, imageUrl: '/maps/battleMap17-sotanoTaberna.png',
        connections: [
    ...
    it('por defecto un mapa no declara conexiones ni restringe salidas', () => {
      const snapshot = buildMap().toSnapshot();
      expect(snapshot.connections).toEqual([]);
      expect(snapshot.closedExits).toBe(false);
      expect(buildMap().canLeadTo('cualquier-mapa')).toBe(true);
    });

    it('un mapa con salidas cerradas solo lleva a sus conexiones (o a sí mismo)', () => {
      const sotano = buildSotano();
      expect(sotano.canLeadTo('cueva-rio')).toBe(true);
      expect(sotano.canLeadTo('tabernaMercenarios')).toBe(true);
      expect(sotano.canLeadTo('sotanoTaberna')).toBe(true);
      // CASO REAL: desde el sótano, el DM inventó una "puerta de hierro" y aplicó la cripta.
      expect(sotano.canLeadTo('cripta-multisala')).toBe(false);
    });

    it('assertCanLeadTo explica a qué mapas se puede ir y por dónde', () => {
      expect(() => buildSotano().assertCanLeadTo('cripta-multisala')).toThrow(
          /cripta-multisala.*no está conectado.*cueva-rio \(por la trampilla candada del Almacén del Sótano 2\)/,
      );
    });

    it('toSnapshot devuelve una copia de connections, no la referencia interna', () => {
      const sotano = buildSotano();
      sotano.toSnapshot().connections.push({ mapId: 'x', via: 'y' });
      expect(sotano.toSnapshot().connections).toHaveLength(2);
    });
  });
});
