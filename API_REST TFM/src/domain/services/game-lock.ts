...
export function withGameLock<T>(gameId: string, fn: () => Promise<T>): Promise<T> {
  ...
  gameLocks.set(gameId, tail);
  // Cuando termina el último trabajo encolado de esta partida, se libera su
  // entrada: antes el mapa crecía sin límite (una entrada por gameId visto).
  void tail.then(() => {
    if (gameLocks.get(gameId) === tail) {
...
/** Nº de partidas con trabajo en cola (solo para tests/diagnóstico). */
export function activeGameLockCount(): number {
