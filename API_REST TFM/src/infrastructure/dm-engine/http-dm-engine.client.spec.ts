...
import { DmEngineRespondedError } from '../../domain/ports/dm-engine.port';
...
describe('HttpDmEngineClient', () => {
  ...
  it('envía el secreto interno en x-internal-api-key si se configura (dm-engine lo exige en /turn)', async () => {
    let sentHeaders: Record<string, string> = {};
    global.fetch = jest.fn(async (_url: string, init: RequestInit) => {
      sentHeaders = init.headers as Record<string, string>;
      return { ok: true, json: async () => ({ narrative: 'Ok', events: [] }) } as Response;
    }) as unknown as typeof fetch;

    await new HttpDmEngineClient('http://dm-engine.local', 'clave-compartida').sendTurn('game-1', []);

    expect(sentHeaders['x-internal-api-key']).toBe('clave-compartida');
  });

  it('no envía la cabecera si no hay secreto configurado', async () => {
    let sentHeaders: Record<string, string> = {};
    global.fetch = jest.fn(async (_url: string, init: RequestInit) => {
      sentHeaders = init.headers as Record<string, string>;
      return { ok: true, json: async () => ({ narrative: 'Ok', events: [] }) } as Response;
    ...
    await new HttpDmEngineClient('http://dm-engine.local').sendTurn('game-1', []);

    expect(sentHeaders['x-internal-api-key']).toBeUndefined();
  });

  it('si la respuesta llega pero su cuerpo no se puede leer, lanza DmEngineRespondedError (no reintentable)', async () => {
    let calls = 0;
    global.fetch = jest.fn(async () => {
      calls++;
      return { ok: true, json: async () => { throw new Error('conexión cortada'); } } as unknown as Response;
    }) as unknown as typeof fetch;

    await expect(new HttpDmEngineClient('http://dm-engine.local').sendTurn('game-1', [])).rejects.toBeInstanceOf(DmEngineRespondedError);
    expect(calls).toBe(1);
  });
});
