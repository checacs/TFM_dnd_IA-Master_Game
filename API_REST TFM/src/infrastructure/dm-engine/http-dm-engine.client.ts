...
import { INTERNAL_API_KEY_HEADER } from '../../interface/internal/internal-api-key';
...
export class HttpDmEngineClient implements DmEngineClient {
  /** internalApiKey: secreto compartido con dm-engine (INTERNAL_API_KEY) -- se envía en cada /turn si está definido. */
  constructor(
  ...
  private async doRequest(gameId: string, messages: DmEngineChatMessage[]): Promise<DmEngineResult> {
    ...
    // Leer el cuerpo también puede fallar (conexión cortada a mitad): dm-engine
    // YA procesó el turno, así que se trata como DmEngineRespondedError para
    // que SendMessageUseCase NO reenvíe el turno entero (duplicaría mutaciones).
    try {
      return (await response.json()) as DmEngineResult;
    } catch (error) {
      throw new DmEngineRespondedError(
        `dm-engine respondió pero no se pudo leer su respuesta: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
}
