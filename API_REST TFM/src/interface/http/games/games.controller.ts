...
import { StartOpeningSceneUseCase } from '../../../application/use-cases/start-opening-scene.use-case';
...
export class GamesController {
  constructor(
    ...
    private readonly startOpeningScene: StartOpeningSceneUseCase,
  ...
  // start-combat y attack: acciones que decide el DM-IA (las usa vía MCP).
  // Ningún cliente las llama por REST, y abiertas a cualquier usuario logueado
  // permitían, p.ej., "atacar" a todo el grupo con attackerModifier:999 y
  // terminar la partida de cualquiera. Quedan solo para admin (pruebas).
  ...
  /**
   * Arranque de la escena inicial (lo dispara ui-web al entrar en una partida
   * en curso sin historia). Antes reenviaba al DM un historial arbitrario del
   * cliente sin comprobar quién llamaba -- ver StartOpeningSceneUseCase. El
   * cuerpo que siga mandando un cliente antiguo se ignora.
   ...
   */
  @Post(':gameId/message')
  sendPlayerMessage(@Param('gameId') gameId: string, @CurrentUserId() requestingUserId: string) {
    return this.startOpeningScene.execute({ gameId, requestingUserId });
  }
