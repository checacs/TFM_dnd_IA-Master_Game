...
import { ResolveEnemyAttackUseCase, ResolveEnemyAttackInput } from '../../application/use-cases/resolve-enemy-attack.use-case';
...
export class GameMcpTools {
  constructor(
    ...
    private readonly resolveEnemyAttack: ResolveEnemyAttackUseCase,
  ...
  resolveEnemyAttackTool(input: ResolveEnemyAttackInput) {
    return this.resolveEnemyAttack.execute(input);
  }

  resolvePlayerAttackTool(input: { gameId: string; attackerCharacterId: string; targetId: string; playerD20: number }) {
    // Sin requestingUserId: la invoca el DM-IA, no el jugador (ver ResolvePlayerAttackInput).
    return this.resolvePlayerAttack.execute(input);
  }
  ...
  endCombatTool(gameId: string) {
    return this.endCombat.execute({ gameId });
  }
  ...
  castSpellTool(gameId: string, casterCharacterId: string, spellId: string, targetId?: string) {
    // Sin requestingUserId: la invoca el DM-IA, no el jugador (ver CastSpellInput).
    return this.castSpell.execute({ gameId, casterCharacterId, spellId, targetId });
  }
}
