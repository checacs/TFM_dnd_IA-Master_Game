...
export function usePlayerRoll(gameId: string) {
  ...
  return useMutation<PlayerRollResult, Error, PlayerRollInput>({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['game', gameId] });
      // La tirada dispara un turno del DM que puede otorgar XP/objetos.
      queryClient.invalidateQueries({ queryKey: ['character'] });
    },
  });
}
...
export function useCharacter(characterId: string | undefined) {
  return useQuery<CharacterSnapshot, Error>({
    ...
    enabled: !!characterId,
    // El DM-IA puede cambiar la ficha en turnos disparados por OTROS jugadores
    // (XP, objetos, daño): antes solo se refrescaba tras una acción propia.
    refetchInterval: 5000,
  });
}
...
export function useLevelUp(characterId: string) {
  const queryClient = useQueryClient();
  // El endpoint no devuelve cuerpo (la ficha se relee al invalidar).
  return useMutation<void, Error, LevelUpInput>({
    mutationFn: (input) => api.post<void>(`/characters/${characterId}/assign-skill-point`, input),
    onSuccess: () => {
...
export function useEquipItem(characterId: string) {
  const queryClient = useQueryClient();
  return useMutation<void, Error, { equipmentId: string }>({
    mutationFn: (input) => api.post<void>(`/characters/${characterId}/equip`, input),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['character', characterId] });
    },
  });
