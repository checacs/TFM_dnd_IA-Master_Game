...
export function CharacterSheetScreen({ route }: Props) {
  ...
  // El HP actual vive en la partida (ver GetCharacterUseCase en la API).
  const unconscious = (game?.players.find((p) => p.characterId === characterId)?.currentHp ?? 1) <= 0;
  // Mientras el DM-IA responde a un turno (de este jugador o de otro) la API
  // rechaza nuevas acciones: antes una segunda acción en ese intervalo se
  // perdía (y la narración del primer turno aparecía dos veces en el chat).
  const dmBusy = !!game?.dmTurnInProgress || playerAction.isPending || playerRoll.isPending;
  const canClaimTurn =
    inCombat && encounter!.roundPhase === 'jugadores' && !hasActed && !isMyTurn && !unconscious && !gameOver;
  const canAct = !gameOver && !dmBusy && !(inCombat && unconscious) && (inCombat ? isMyTurn : isCaptain);
  const waitLabel = gameOver ? 'Partida terminada' : dmBusy ? 'El DM está respondiendo...' : 'Espera tu turno';
  ...
  if (gameOver) {
    combatStatusText = 'La partida ha terminado: todo el grupo ha caído.';
  } else if (inCombat && unconscious) {
    combatStatusText = 'Tu personaje está inconsciente (0 HP) y no puede actuar.';
  } else if (dmBusy) {
    combatStatusText = 'El DM está respondiendo... espera a que termine para actuar.';
  } else if (!inCombat) {
      ...
      >
            ...
            <View style={styles.actionButtonsRow}>
              ...
              >
                ...
                <Text style={styles.diceButtonText}>
                  {playerRoll.isPending ? 'Tirando...' : canAct ? 'Tirar Dados' : waitLabel}
                </Text>
              </Pressable>
            </View>
            {lastRoll && (
        ...
        <View style={[styles.actionBar, { paddingBottom: Math.max(16, insets.bottom + 8) }]}>
          <TextInput
            ...
            placeholder={canAct ? '¿Qué hace tu personaje?' : `${waitLabel}...`}
              ...
              <View style={styles.section}>
                ...
                {character.inventory.map((item) => {
                  const isEquipped =
                    ...
                    item.equipmentId === character.equippedShieldId ||
