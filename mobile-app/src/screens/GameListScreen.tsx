...
export function GameListScreen({ navigation }: Props) {
  ...
  const handleJoinByCode = () => {
    ...
    if (!trimmed) return;
    // Los códigos se generan en mayúsculas y la búsqueda es exacta ("k7m2qx"
    // daba "Partida no encontrada"); los ids UUID antiguos se dejan tal cual.
    const code = /^[a-z0-9]{4,8}$/i.test(trimmed) ? trimmed.toUpperCase() : trimmed;
    navigation.navigate('GameDetail', { gameId: code });
    setJoinCode('');
  };
