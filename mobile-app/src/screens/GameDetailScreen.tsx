...
/** Igual que MIN_PLAYERS_TO_LAUNCH en Game (API). */
...
export function GameDetailScreen({ route, navigation }: Props) {
  ...
  // Solo hay sala de espera mientras la partida está en configuración: antes
  // una partida ya empezada o terminada seguía mostrando "Unirse" e "Iniciar".
  const inLobby = game?.status === 'configuracion';
  const isFull = !!game && game.players.length >= game.maxPlayers;
  // Mismas reglas que Game.launch en la API (mínimo 2 jugadores y un capitán
  // válido, o el host jugando): antes el botón se habilitaba con 1 jugador.
  const hostIsPlayer = !!game && game.players.some((p) => p.userId === game.hostUserId);
  const canLaunch = !!game && game.players.length >= MIN_PLAYERS_TO_LAUNCH && (!!game.captainUserId || hostIsPlayer);

  useEffect(() => {
    if ((game?.status === 'en_curso' || game?.status === 'finalizada') && myPlayer) {
    ...
    >
      <ScrollView contentContainerStyle={styles.scroll}>
        <View style={styles.card}>
          ...
          {!inLobby && !myPlayer && (
            <Text style={styles.subtitle}>
              {game.status === 'finalizada' ? 'Esta partida ya ha terminado.' : 'Esta partida ya ha empezado.'}
            </Text>
          )}

          {inLobby && isFull && !myPlayer && <Text style={styles.subtitle}>La partida está completa.</Text>}

          {inLobby && !isFull && userId && !myPlayer && !showJoinForm && (
          ...
          {isHost && inLobby && (
            <Pressable
              style={[
                styles.button,
                styles.launchButton,
                (!canLaunch || launchGame.isPending) && styles.buttonDisabled,
              ]}
              onPress={() => launchGame.mutate()}
              disabled={!canLaunch || launchGame.isPending}
            >
              <Text style={styles.buttonText}>{launchGame.isPending ? 'Iniciando...' : 'Iniciar partida'}</Text>
            </Pressable>
          )}
          {isHost && inLobby && !canLaunch && (
            <Text style={styles.subtitle}>
              {game.players.length < MIN_PLAYERS_TO_LAUNCH
                ? `Hacen falta al menos ${MIN_PLAYERS_TO_LAUNCH} jugadores para iniciar.`
                : 'Asigna un capitán (desde ui-web) antes de iniciar.'}
            </Text>
          )}
