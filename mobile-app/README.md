...
En React Native `localhost` apunta al propio dispositivo, no al ordenador donde corre el backend. Por eso, si no defines `EXPO_PUBLIC_API_URL`, la app deduce la URL de la API sola (`src/utils/api-base.ts`):

- **Expo Go / dev client:** la IP del PC que sirve el bundle, puerto 3000 (`http://192.168.X.X:3000`).
- **Modo web** (navegador del PC o del móvil): el mismo host desde el que se abrió la página, puerto 3000.
- En cualquier otro caso, `http://localhost:3000`.

Para apuntar a otra API (p. ej. la de Render), define la variable antes de arrancar; los perfiles `preview`/`production` de `eas.json` ya la fijan:

```bash
EXPO_PUBLIC_API_URL=https://mi-api.example.com npx expo start
``` **Sin sufijo `/api`**: a diferencia de `ui-web` (que llama a `/api/*` y depende del proxy de Vite para reescribir esa ruta antes de reenviarla), `mobile-app` llama directo al backend sin proxy, y el backend no tiene ningún prefijo global — sus rutas son `/auth/login`, `/games`, etc.
